...
const Ranking = () => {
  ...
  const [defaultActiveKey, setDefaultActiveKey] = useState(
    user && user.gender === "m" ? "ms" : "ws"
  );
  const dispatch = useDispatch();
  const { isLoading } = ranking;
  const data = ranking.data || { ms: [], ws: [] };
  const types = [
    { type: "Meesüksikmängud", short: "ms" },
    { type: "Naisüksikmängud", short: "ws" },
  ];
  ...
  const handleJoinLeave = (decision) => {
    services
      .entryRankings(decision)
      .then((res) => {
        dispatch(updateRankings());
        message.success(
          `Oled ${decision.toLowerCase().concat("nud")} edetabeli${
            decision === "Liitu" ? "ga" : "st"
          }`
        );
      })
      .catch((e) =>
        message.error(`Viga edetabelist ${decision.toLowerCase()}misega`)
      );
  };
    ...
    return (
      <div style={{ display: "flex" }}>
        <Popconfirm
          ...
          title={`Oled kindel, et tahad ${decision
            .toLowerCase()
            .concat("da")} edetabeli${ending}?`}
          ...
          {types.map((e, i) => (
            <TabPane tab={e.type} key={types[i].short}>
              <RankingTable
                ...
                type={e.short}
                data={data[e.short]}
                displayJoinLeaveButton={displayJoinLeaveButton}
              />
            </TabPane>
          ))}
        </Tabs>
